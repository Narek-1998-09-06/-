...
class APPMAKER_WC_REST_Products_Controller extends APPMAKER_WC_REST_Posts_Abstract_Controller {
    ...
    public function get_product_data( $product_obj, $expanded = false ) {
        ...
        if ( $product_obj->get_type() === 'variable' ) {           
            ...
            $variation_price = $this->variable_product_price( $product );

            if ( $product_obj->is_on_sale() ) {

                $variation_regular_price = $this->variable_sale_product_regular_price_display( $product ); 

                $data['regular_price']         = $variation_regular_price;
                $data['regular_price_display'] = APPMAKER_WC_Helper::get_display_price($variation_regular_price );
                $data['price']                 = $variation_price;
                $data['price_display']         = APPMAKER_WC_Helper::get_display_price($variation_price);
                $data['sale_price']            = $variation_price;
                $data['sale_price_display']    = APPMAKER_WC_Helper::get_display_price($variation_price);
            } else {
                $data['price']                 = $variation_price;
                $data['price_display']         = APPMAKER_WC_Helper::get_display_price($variation_price);
                $data['regular_price_display'] = APPMAKER_WC_Helper::get_display_price($variation_price);
            }
        }
    ...
    protected function variable_sale_product_regular_price_display( $product ) {
        ...
        if ( $product->get_price() === '' || empty( $prices['price'] ) ) {
            ...
            $price             = $min_regular_price !== $max_regular_price ? sprintf( _x( '%1$s-%2$s', '', 'woocommerce' ), $min_regular_price , $max_regular_price  ) :  $min_regular_price ;
        }
