...
class APPMAKER_WC_Dynamic_form {
	...
	private function add_field( $args, $key, $section ) {
        ...
        if(!empty($this->response['items'][ $key ]['options']) && count($this->response['items'][ $key ]['options'])==1 && $this->response['items'][ $key ]['id'] != 'billing_state' && $this->response['items'][ $key ]['type'] == 'select'){
